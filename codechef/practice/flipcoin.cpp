...
inline void update(int node, int p, int q, int a, int b)
{
	...
	{
		int calc;
		calc=query(1,0,n-1,a,b);
		arr[node]-=calc;
	}
	else if(p>=a && p<=b && q>b)
	{
		int calc;
		calc=query(1,0,n-1,p,b);
		arr[node]-=calc;
	}
	else if(q>=a && q<=b && p<a)
	{
		int calc;
		calc=query(1,0,n-1,a,q);
		arr[node]-=calc;
	}
...
int main()
{
	...
	rep(i,q)
	{
		...
		if(val==0)
		{
			update(1,0,n-1,val1,val2);
		}
		else
		{
			calc=val2-val1+1-query(1,0,n-1,val1,val2);
			pin(calc);
			//cout<<calc<<endl;
			//ansit.pb(calc);
			//fflush(stdout);
